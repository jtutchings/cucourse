...
function cucourse_get_tutors($courses, $unicourses, $modules) {
    ...
    foreach ($courses as $course) {
        ...
        foreach ($users as $user) {
            if (!isset($tutors[$user->userid])) {
                $tutors[$user->userid] = $DB->get_record('user', array('id' => $user->userid));
                $tutors[$user->userid]->userid = $user->userid;

                $t = user_get_user_details($tutors[$user->userid]);
                if (isset($t['customfields'])) {
                    foreach ($t['customfields'] as $f) {
                        if ($f['shortname'] == CUCOURSE_OFFICE_HOURS) {
                            $tutors[$user->userid]->officehours = $f['value'];
                        }
                    }
                } else {
                    $tutors[$user->userid]->officehours = get_string('officehournotset', 'block_cucourse');
                }
            }

            $tutors[$user->userid]->courses[] = html_writer::link(new moodle_url('/course/view.php',
                                                                    array('id' => $course->id)),
                                                                    $course->fullname);
            if (isset($unicourses[$course->shortname])) {
                $coursetutors[$user->userid] = true;
            } else if (isset($modules[$course->shortname])) {
                $currenttutors[$user->userid] = true;
            }
        }
    }
    ...
    if ($currenttutors) {
        ...
        foreach (array_keys($currenttutors) as $tutorid) {
            if (isset($tutors[$tutorid]) && !$tutorseen[$tutorid]) {
                $data[] = cucourse_format_tutor_details($tutors[$tutorid]);
                $tutorseen[$tutorid] = true;
            }
        }
